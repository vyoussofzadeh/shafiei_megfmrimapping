...
if exist(fullfile(outd.sub,['bic_',subj,'.mat']), 'file') == 2
    load(fullfile(outd.sub,['bic_',subj,'.mat']));
else
    
    if flag.preprocessing.filtering == 1
        %% Filteting, Event reading, Artifact rejecrtion
        savepath = fullfile(outd.sub,['bf_',subj,'.mat']);
    ...
    if flag.preprocessing.artifact == 1
        ...
        savepath = fullfile(outd.sub,['ba_',subj,'.mat']);
        cfg = [];
...
%         cfg.savepath = fullfile(outd.sub,['ic_',subj,'.mat']);
        ...
        cfg.savefile = fullfile(cfg.savepath,['bic_',cfg.subj,'.mat']);
