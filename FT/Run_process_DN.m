...
for i = 1:size(datafile1,1)
    ...
    %% Time-freq analysis (tfr, trial)
%     cfg = [];
%     cfg.savefile = [];
%     cfg.saveflag = 2;
%     cfg.lay  = lay;
%     tfr_trl = vy_tfr_trl(cfg, cln_data);
    %
    cfg = [];
...
%             toi = [-0.3,0;1,1.5];
    ...
    if ~isempty(d)
        ...
        cfg.subj = subj;
        cfg.plotflag = 2;
        [mri_realigned,individual_seg,individual_headmodel,headshape, individual_grid_8mm, individual_grid_10mm] = vy_mri_neuromag1(cfg);
    end
